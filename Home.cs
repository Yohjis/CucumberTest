...
namespace TestingEpam
{
    public class Home
    {
        ...
        {
            new WebDriverWait(_driver, TimeSpan.FromSeconds(10)).Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(logoButton));
            logoButton.Click();
            return new Home(_driver);
        }

        public Home OpenWorkPage()
        {
            new WebDriverWait(_driver, TimeSpan.FromSeconds(10)).Until(SeleniumExtras.WaitHelpers.ExpectedConditions.ElementToBeClickable(workTabButton));
            workTabButton.Click();
            return new Home(_driver);
        }
